...
# ============================================================
# CONFIG LOADING (makes this script run independently)
# ============================================================
# Default config file lives next to this script. A different
# path can be passed as the first command-line argument.
DEFAULT_CONFIG_FILE = Path(__file__).resolve().parent / 'config.properties'


def load_config(config_path=None):
    """Read key=value settings from a .properties file.

    Lines starting with '#' or '!' are treated as comments and blank
    lines are ignored. Both '=' and ':' are accepted as separators.
    Returns a dict of upper-cased keys to string values.
    """
    config_path = Path(config_path) if config_path else DEFAULT_CONFIG_FILE

    if not config_path.exists():
        raise FileNotFoundError(
            f"Config file not found: {config_path}\n"
            f"Create a 'config.properties' file (see the sample) next to this script."
        )
    ...
    with open(config_path, 'r', encoding='utf-8') as f:
        for raw_line in f:
            ...
            key = line[:sep_index].strip().upper()
...
def _get(settings, key, default=None):
    """Fetch a config value, returning default when missing or blank."""
    value = settings.get(key.upper(), '')
    value = value.strip() if isinstance(value, str) else value
    return value if value else default


def build_run_args(settings):
    """Translate raw config values into arguments for generate_audio_files()."""
    excel_path = _get(settings, 'EXCEL_PATH')
    output_dir = _get(settings, 'OUTPUT_DIR', 'tts-output')
    tts_service = _get(settings, 'TTS_SERVICE', 'google').lower()
    creds_path = _get(settings, 'GOOGLE_CREDS_PATH')
    azure_key = _get(settings, 'AZURE_SPEECH_KEY')
    azure_region = _get(settings, 'AZURE_SPEECH_REGION')

    # LIMIT: optional integer; blank or 0 means "process all"
    limit_raw = _get(settings, 'LIMIT')
    limit = None
    if limit_raw is not None:
        try:
            ...
            raise ValueError(f"LIMIT must be an integer, got: '{limit_raw}'")

    # ---- Validation ----
    errors = []
    if not excel_path:
        errors.append("EXCEL_PATH is required (path to the input Excel file).")
    elif not Path(excel_path).exists():
        errors.append(f"EXCEL_PATH does not exist: {excel_path}")

    if tts_service not in ('google', 'azure', 'edge'):
        errors.append(f"TTS_SERVICE must be one of google|azure|edge, got: '{tts_service}'")

    if tts_service == 'google' and creds_path and not Path(creds_path).exists():
        errors.append(f"GOOGLE_CREDS_PATH does not exist: {creds_path}")

    if tts_service == 'azure' and (not azure_key or not azure_region):
        errors.append("Azure TTS requires both AZURE_SPEECH_KEY and AZURE_SPEECH_REGION.")
...
# ============================================================
def main():
    """Run the generator standalone, reading all inputs from config.properties."""
    # Allow an optional config path override: python tts_generator.py my_config.properties
    config_path = sys.argv[1] if len(sys.argv) > 1 else None

    print("=" * 60)
    print("🚀 TTS GENERATOR (standalone mode)")
    print("=" * 60)

    try:
        settings = load_config(config_path)
        args = build_run_args(settings)
    except (FileNotFoundError, ValueError) as e:
        print(f"[ERROR] {e}")
        sys.exit(1)

    print(f"  Config file : {config_path or DEFAULT_CONFIG_FILE}")
    print(f"  Excel input : {args['excel_path']}")
    print(f"  Output dir  : {args['output_dir']}")
    print(f"  TTS service : {args['tts_service']}")
    print(f"  Limit       : {args['limit'] if args['limit'] else 'ALL'}")
    print("")

    summary = generate_audio_files(
    ...
    if summary:
        print("✅ DONE")
        print("=" * 60)
        print(f"  Files generated : {summary['total_files']}/{summary['total_expected']}")
        print(f"  Utterances      : {summary['utterances_processed']}")
        print(f"  Audio directory : {summary['audio_directory']}")
    else:
